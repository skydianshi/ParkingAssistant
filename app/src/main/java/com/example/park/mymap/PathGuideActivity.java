...
public class PathGuideActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent != null) {
			list=(ArrayList<BNRoutePlanNode>)intent.getSerializableExtra(NaViPathActivity.ROUTE_PLAN_NODE);//接收到路线规划得到的节点集合
	...
	private void addCustomizedLayerItems() {//添加自定义的图层，可以更换导航的中心的图标
		...
		if (mBNRoutePlanNode != null) {
			...
			item1 = new CustomizedLayerItem(mBNRoutePlanNode.getLongitude(), mBNRoutePlanNode.getLatitude(),
					CoordinateType.BD09LL, getResources().getDrawable(R.mipmap.ic_launcher), CustomizedLayerItem.ALIGN_CENTER);
			items.add(item1);
	...
	public  void toast(String str){
		Toast.makeText(PathGuideActivity.this, str, Toast.LENGTH_SHORT).show();
	}
}
